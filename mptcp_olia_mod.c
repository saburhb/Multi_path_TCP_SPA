...
static u32 mptcp_get_cwnd_bestpath_comparator(struct mptcp_cb *mpcb, struct tcp_sock *curr_tp, struct mptcp_olia *curr_ca)
{
	struct mptcp_olia *ca;
	struct tcp_sock *tp;
	struct sock *sk;
	u64 tmp_int, tmp_rtt, curr_rtt, best_int = 0, curr_int = 0, best_rtt = 1;
	u32 cwnd_bestpath = 1, tmp_cwnd;
	u32 c, comparator, ratio = 1;
	u8 index;

	curr_rtt = curr_tp->srtt * curr_tp->srtt;
	curr_int = max(curr_ca->mptcp_loss3 -  curr_ca->mptcp_loss2, curr_ca->mptcp_loss2 - curr_ca->mptcp_loss1);
	...
	mptcp_for_each_sk(mpcb, sk) {
		...
		tmp_rtt = tp->srtt * tp->srtt;
		/* TODO - check here and rename variables */
		tmp_int = max(ca->mptcp_loss3 - ca->mptcp_loss2, ca->mptcp_loss2 - ca->mptcp_loss1);

		tmp_cwnd = mptcp_get_crt_cwnd(sk);
		if(tmp_int * best_rtt >= best_int * tmp_rtt) {
			best_rtt = tmp_rtt;
			best_int = tmp_int;
			cwnd_bestpath = tmp_cwnd;
		}
	}
	
	c = div_u64((best_int * curr_rtt), (curr_int * best_rtt));
	for(index = 1; index < 10; index++)
	{
		if( ((index * index) <= c) && (((index+1) * (index+1)) >= c) )
		{
			ratio = index;
		}
	}
	comparator = div_u64(cwnd_bestpath, ratio);
	return comparator;
	...
	if (sysctl_tcp_abc) {
		...
		if (ca->mptcp_snd_cwnd_cnt >= (1 << scale) - 1) {
			if (tp->snd_cwnd < tp->snd_cwnd_clamp)
				tp->snd_cwnd++;
			ca->mptcp_snd_cwnd_cnt = 0;
			cwnd_comp = mptcp_get_cwnd_bestpath_comparator(mpcb, tp, ca);
		        if(tp->snd_cwnd < cwnd_comp)
                	tp->snd_cwnd = cwnd_comp;
...
u32 mptcp_olia_min_cwnd(const struct sock *sk)
{
	u32 cwnd_comp = 0;
	...
	struct mptcp_cb *mpcb = tp->mpcb;
	cwnd_comp = mptcp_get_cwnd_bestpath_comparator(mpcb, tp, ca);
	tp->snd_cwnd = max((int) 1 , (int) cwnd_comp);	
